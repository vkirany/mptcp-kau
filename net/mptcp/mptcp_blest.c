...
static void modify_lambda(struct sock *meta_sk, int dir)
{
	...
	struct blestsched_priv *dsp = blestsched_get_priv(tp);
	int delta = 0;
	u32 old_lambda_1000;
...
static void update_lambda(struct sock *slow_sk, struct sock *meta_sk)
{
	...
	struct blestsched_priv *slowdsp = blestsched_get_priv(slowtp);
	struct tcp_sock *tp = tcp_sk(meta_sk);
	struct blestsched_priv *dsp = blestsched_get_priv(tp);
	u32 slowrtt;
...
static u32 estimate_bytes(struct sock* sk, u32 time_8)
{
	struct tcp_sock *tp = tcp_sk(sk);
	struct blestsched_priv *dsp = blestsched_get_priv(tp);
	u32 bytes, num_rtts, packets, lamb_1000, avg_rtt;

	lamb_1000 = get_dyn_lambda(mptcp_meta_sk(sk));

	avg_rtt = (dsp->min_srtt + dsp->max_srtt) / 2;
	if (avg_rtt == 0)
		num_rtts = 1; // sanity
	else
...
static bool mptcp_blest_is_available(struct sock *sk, struct sk_buff *skb,
 ...
 */
static struct sock *get_blest_available_subflow(struct sock *meta_sk,
					  struct sk_buff *skb,
					  bool zero_wnd_test)
{
	...
	if (mpcb->cnt_subflows == 1) {
		...
		if (!mptcp_blest_is_available(bestsk, skb, zero_wnd_test, true))
			bestsk = NULL;
		return bestsk;
	}
	...
	if (meta_sk->sk_shutdown & RCV_SHUTDOWN &&
		...
		mptcp_for_each_sk(mpcb, sk) {
			if (tcp_sk(sk)->mptcp->path_index == mpcb->dfin_path_index &&
			    mptcp_blest_is_available(sk, skb, zero_wnd_test, true))
				return sk;
		}
	}
	...
	if (skb) { // BLEST: need an SKB
		...
		mptcp_for_each_sk(mpcb, sk) {
			...
			if (tp->srtt_us < best_time_to_peer) {
				if (!mptcp_blest_is_available(sk, skb, zero_wnd_test, true))
					continue;
		...
		if (bestsk && bestsk != minsk) {
			struct tcp_sock *meta_tp, *mintp, *besttp;
			u32 slow_linger_time, fast_bytes, slow_bytes, slow_inflight_bytes, avail_space;
			...
			slow_linger_time = estimate_linger_time(bestsk);
			fast_bytes = estimate_bytes(minsk, slow_linger_time);
...
{
	struct sock *meta_sk;
	struct tcp_sock *tp = tcp_sk(sk), *tp_it, *metatp;
	struct sk_buff *skb_head;
	struct blestsched_priv *dsp = blestsched_get_priv(tp);
	struct blestsched_priv *metadsp;
	...
	metatp = tcp_sk(meta_sk);
	metadsp = blestsched_get_priv(metatp);
...
retrans:
	...
	if (!(TCP_SKB_CB(skb_head)->path_mask & mptcp_pi_to_flag(tp->mptcp->path_index))) {
		...
		if (do_retrans && mptcp_blest_is_available(sk, skb_head, false, true)) {
			return skb_head;
		}
	}
	return NULL;
	...
	if (skb) {
		*reinject = 1;
		...
		if (!skb && meta_sk->sk_socket &&
			...
			struct sock *subsk = get_blest_available_subflow(meta_sk, NULL,
								   false);
			if (!subsk)
				return NULL;
					  ...
					  unsigned int *limit)
{
	...
	*subsk = get_blest_available_subflow(meta_sk, skb, false);
	if (!*subsk)
		return NULL;
...
{
	struct blestsched_priv *dsp = blestsched_get_priv(tcp_sk(sk));
...
static struct mptcp_sched_ops mptcp_blest = {
	.get_subflow = get_blest_available_subflow,
