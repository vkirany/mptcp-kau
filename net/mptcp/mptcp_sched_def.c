...
static struct defschedmod_priv *defschedmod_get_priv(const struct tcp_sock *tp)
{
...
static bool mptcp_is_defmod_available(struct sock *sk, const struct sk_buff *skb,
			bool zero_wnd_test)
{
	return !mptcp_is_defmod_unavailable(sk) &&
...
static bool subflow_is_defmod_backup(const struct tcp_sock *tp)
{
	return tp->mptcp->rcv_low_prio || tp->mptcp->low_prio;
}

static bool subflow_is_defmod_active(const struct tcp_sock *tp)
{
	return !tp->mptcp->rcv_low_prio && !tp->mptcp->low_prio;
}
			    ...
			    bool zero_wnd_test, bool *force)
{
	...
	mptcp_for_each_sk(mpcb, sk) {
		...
		if (mptcp_is_defmod_unavailable(sk)) {
 ...
 */
struct sock *get_available_defmod_subflow(struct sock *meta_sk, struct sk_buff *skb,
				   bool zero_wnd_test)
{
	...
	if (mpcb->cnt_subflows == 1) {
		...
		if (!mptcp_is_defmod_available(sk, skb, zero_wnd_test)) {
	...
	if (meta_sk->sk_shutdown & RCV_SHUTDOWN &&
		...
		mptcp_for_each_sk(mpcb, sk) {
			if (tcp_sk(sk)->mptcp->path_index == mpcb->dfin_path_index &&
			    mptcp_is_defmod_available(sk, skb, zero_wnd_test))
				return sk;
		}
	}

	/* Find the best subflow */
	sk = get_subflow_from_selectors(mpcb, skb, &subflow_is_defmod_active,
					zero_wnd_test, &force);
	if (force)
	...
	sk = get_subflow_from_selectors(mpcb, skb, &subflow_is_defmod_backup,
					zero_wnd_test, &force);
	if (!force && skb)
...
static struct sk_buff *mptcp_rcv_buf_optimization(struct sock *sk, int penal)
{
	...
	struct defschedmod_priv *dsp = defschedmod_get_priv(tp);
...
retrans:
	...
	if (!(TCP_SKB_CB(skb_head)->path_mask & mptcp_pi_to_flag(tp->mptcp->path_index))) {
		...
		if (do_retrans && mptcp_is_defmod_available(sk, skb_head, false))
			return skb_head;
	}
	return NULL;
}
	...
	if (skb) {
		*reinject = 1;
		...
		if (!skb && meta_sk->sk_socket &&
			...
			struct sock *subsk = get_available_defmod_subflow(meta_sk, NULL,
								   false);
			if (!subsk)
				return NULL;
					  ...
					  unsigned int *limit)
{
	...
	*subsk = get_available_defmod_subflow(meta_sk, skb, false);
	if (!*subsk)
		return NULL;
...
static void defschedmod_init(struct sock *sk)
{
	struct defschedmod_priv *dsp = defschedmod_get_priv(tcp_sk(sk));

	dsp->last_rbuf_opti = tcp_time_stamp;
}

struct mptcp_sched_ops mptcp_sched_defmod = {
	.get_subflow = get_available_defmod_subflow,
	.next_segment = mptcp_next_segment,
	.init = defschedmod_init,
	.name = "defaultmod",
	.owner = THIS_MODULE,
};

/* Set default value from kernel configuration at bootup */
static int __init defaultmod_register(void)
{
	BUILD_BUG_ON(sizeof(struct defschedmod_priv) > MPTCP_SCHED_SIZE);

	if (mptcp_register_scheduler(&mptcp_sched_defmod))
		return -1;

	return 0;
}

static void defaultmod_unregister(void)
{
	mptcp_unregister_scheduler(&mptcp_sched_defmod);
}

module_init(defaultmod_register);
module_exit(defaultmod_unregister);

MODULE_AUTHOR("Per Hurtig");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("DEFALUT MOD SCHEDULER FOR MPTCP");
MODULE_VERSION("0.90");
