...
struct poorsched_priv {
	u32	last_rbuf_opti;
};

static struct poorsched_priv *poorsched_get_priv(const struct tcp_sock *tp)
{
	return (struct poorsched_priv *)&tp->mptcp->mptcp_sched[0];
}
 ...
 */
struct sock *get_poor_subflow(struct sock *meta_sk, struct sk_buff *skb,
				   bool zero_wnd_test)
{
...
static struct sk_buff *mptcp_rcv_buf_optimization(struct sock *sk, int penal)
{
	...
	struct poorsched_priv *dsp = poorsched_get_priv(tp);
	...
	if (skb) {
		*reinject = 1;
		...
		if (!skb && meta_sk->sk_socket &&
			...
			struct sock *subsk = get_poor_subflow(meta_sk, NULL,
								   false);
			if (!subsk)
				return NULL;
					  ...
					  unsigned int *limit)
{
	...
	*subsk = get_poor_subflow(meta_sk, skb, false);
	if (!*subsk)
		return NULL;
...
static void poorsched_init(struct sock *sk)
{
...
struct mptcp_sched_ops mptcp_sched_poor = {
	.get_subflow = get_poor_subflow,
...
static int __init poorsched_register(void)
{
	BUILD_BUG_ON(sizeof(struct poorsched_priv) > MPTCP_SCHED_SIZE);

	if (mptcp_register_scheduler(&mptcp_sched_poor))
		return -1;

	return 0;
}

static void poorsched_unregister(void)
{
	mptcp_unregister_scheduler(&mptcp_sched_poor);
}

module_init(poorsched_register);
module_exit(poorsched_unregister);

MODULE_AUTHOR("Per Hurtig");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("POOR SCHEDULER");
MODULE_VERSION("0.91");
